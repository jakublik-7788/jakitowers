...
import { rapSongs as dailySongs } from "@/app/scripts/songs/rap/rapSongs";
...
const LS_DAY_PREFIX = "jakitowers_day_rap_";
const LS_RESULTS_RAP_KEY = "jakitowers_rap_results";
const LS_RULES_SEEN_KEY = "jakitowers_rules_seen_rap";
...
function saveRapResults(results: Record<number, "win" | "lose">): void {
  try {
    localStorage.setItem(LS_RESULTS_RAP_KEY, JSON.stringify(results));
    window.dispatchEvent(new CustomEvent("gameResult"));
  } catch {}
}

function loadRapResults(): Record<number, "win" | "lose"> {
  try {
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-white/8">
          ...
          <input
            ...
            className="w-20 h-1.5 rounded-full appearance-none cursor-pointer bg-zinc-700
...
// ─── Panel Wyników po zakończeniu gry ─────────────────────────────────────────
const ResultPanel = ({
  status,
  attempts,
  currentDay,
...
export default function RapPage() {
  ...
  const [rapResults, setRapResults] = useState<Record<number, "win" | "lose">>(
    {},
  );
  ...
  useEffect(() => {
    ...
    if (dayParam) {
      ...
      if (!isNaN(d) && d >= 1 && d <= dailySongs.length) setCurrentDay(d);
    }
    try {
      const c =
  ...
  const handleGuess = () => {
    ...
    const exactMatch = rapAllSongs.some(
    ...
    const songInDb = rapAllSongs.find(
      (s) => norm(s.artist + " - " + s.title) === norm(inputValue.trim()),
    );
    ...
    if (status === "correct") {
      setIsFinished(true);
      const r = { ...rapResults, [currentDay]: "win" as const };
      setRapResults(r);
      saveRapResults(r);
      ...
      setIsFinished(true);
      const r = { ...rapResults, [currentDay]: "lose" as const };
      setRapResults(r);
      saveRapResults(r);
  ...
  return (
    <div
      className="min-h-dvh bg-zinc-950 text-white flex flex-col overflow-hidden relative"
    ...
    >
        ...
        <div className="w-full md:w-1/2 flex items-center justify-center mb-4 md:mb-0">
          {!isFinished ? (
            ...
            <ResultPanel
              ...
              onPlayKlasyki={() => {
                play("click");
                router.push(`/gra/daily/klasyki?day=${currentDay}`);
              }}
              onBackToMenu={() => {
                play("click");
                router.push("/");
      ...
      <AnimatePresence>
        {showRules && (
          <RulesModal modeKey="rap-daily" onClose={handleCloseRules} />
        )}
      </AnimatePresence>
      <Footer />
    </div>
