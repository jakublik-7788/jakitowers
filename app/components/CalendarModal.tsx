...
export const CalendarModal = ({
  ...
  const rawFirst = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), 1).getDay();
  ...
  const isNextMonthDisabled =
    currentMonth.getMonth() === todayDate.getMonth() && currentMonth.getFullYear() === todayDate.getFullYear();
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="bg-zinc-950 border-2 border-accent/40 rounded-[30px] p-6 shadow-[0_0_50px_var(--accent-glow)] max-md:p-4">
              <div className="flex items-center justify-between mb-6 max-md:mb-4">
                <motion.button
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={handlePrevMonth}
                  className="text-zinc-400 hover:text-accent transition-colors max-md:w-10 max-md:h-10"
                >
                  <svg
                    width="24"
                    height="24"
                ...
                <h2 className="text-xl font-black text-white tracking-wider max-md:text-lg">
              ...
              <div className="grid grid-cols-7 gap-1 mb-2">
                {["PN", "WT", "ŚR", "CZ", "PT", "SB", "ND"].map((d) => (
                  <div key={d} className="text-center text-zinc-500 text-xs font-black tracking-wider max-md:text-[10px]">
                    {d}
                  </div>
                ))}
              </div>
                  ...
                  return (
                    <motion.button
                      ...
                      className={`
                        ...
                        transition-all duration-200 max-md:text-xs max-md:rounded-md
                        ${
                          isFuture
                    ...
                    >
                      ...
                      {!isFuture && dayNumber !== null && (
                        <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 text-[6px] font-bold text-zinc-700 leading-none max-md:text-[5px]">
                          #{dayNumber}
                        </span>
                      )}
                      {isToday && <span className="absolute -top-1 -right-1 w-2 h-2 bg-accent rounded-full animate-pulse" />}
                    </motion.button>
                  );
                })}
              </div>

              <div className="flex items-center justify-center gap-6 mt-6 pt-4 border-t border-white/5 max-md:gap-3 max-md:mt-4 max-md:pt-3">
                <div className="flex items-center gap-2 max-md:gap-1">
                  <div className="w-3 h-3 rounded-full bg-green-500/50 shadow-[0_0_8px_rgba(34,197,94,0.5)] max-md:w-2.5 max-md:h-2.5" />
                  <span className="text-xs text-zinc-400 max-md:text-[10px]">Wygrana</span>
                </div>
                <div className="flex items-center gap-2 max-md:gap-1">
                  <div className="w-3 h-3 rounded-full bg-red-500/50 shadow-[0_0_8px_rgba(239,68,68,0.5)] max-md:w-2.5 max-md:h-2.5" />
                  <span className="text-xs text-zinc-400 max-md:text-[10px]">Przegrana</span>
                </div>
                <div className="flex items-center gap-2 max-md:gap-1">
                  <div className="w-3 h-3 rounded-full bg-accent shadow-[0_0_8px_var(--accent-glow)] max-md:w-2.5 max-md:h-2.5" />
                  <span className="text-xs text-zinc-400 max-md:text-[10px]">Dzisiaj</span>
                </div>
              </div>
            </div>
          </motion.div>
