...
export const ShareCard = ({ status, currentSong, cardRef, dayNumber, attempts }: ShareCardProps) => {
  ...
  return (
    <>
      <button
        ...
        className="flex items-center justify-center gap-3 w-full bg-white/5 border border-white/10 text-white py-4 rounded-2xl font-black tracking-widest hover:bg-white/10 active:scale-95 transition-all uppercase italic relative overflow-hidden max-md:py-3 max-md:gap-2 max-md:text-sm"
      >
        {copied ? (
          <>
            <Check size={20} className="text-green-400 shrink-0 max-md:w-5 max-md:h-5" />
            <span className="text-green-400">Link skopiowany!</span>
          </>
        ) : (
          <>
            <Share2 size={20} className="text-accent shrink-0 max-md:w-5 max-md:h-5" />
            Pobierz wynik
          </>
        )}
      </button>
      ...
      {!copied && (
        <p className="text-zinc-700 text-[10px] text-center -mt-1 tracking-wide max-md:text-[9px]">
      ...
      <div className="fixed left-[-9999px] top-0">
        ...
        >
          ...
          <div className="relative z-10 text-center">
            ...
            {isWin && (
              <div className="mb-10">
                <p className="text-white text-7xl font-black italic tracking-tighter leading-none">{attempts}/5</p>
            ...
            <div
              className={`px-12 py-4 rounded-full border-4 inline-block ${
                isWin ? "border-green-400/30 text-green-400" : "border-red-500/30 text-red-500"
              }`}
            >
          ...
          <div className="relative z-10 w-full bg-zinc-900/60 border-2 border-white/10 rounded-[50px] p-16 backdrop-blur-3xl mb-12">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-accent text-4xl font-bold uppercase tracking-[0.2em]">{currentSong?.artist}</p>
            </div>
          </div>

          <div className="relative z-10 opacity-30">
            <p className="text-white text-3xl font-bold tracking-[0.6em] uppercase">{SITE_URL.replace("https://", "")}</p>
          </div>
        </div>
      </div>
    </>
