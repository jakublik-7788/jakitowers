...
import { Volume2, Volume, Volume1, VolumeX, Calendar, Infinity, Sun, Settings } from "lucide-react";
...
export const Header = ({
  ...
  return (
    <div className="relative z-50 pt-12 pb-16 flex flex-col items-center bg-gradient-to-b from-black via-black/90 to-transparent">
      ...
      <div className="absolute top-8 left-8 flex gap-4 text-zinc-700 items-center">
        ...
        >
          <motion.button
            ...
            style={{ 
              boxShadow: isVolumeHovered ? "0 0 15px var(--accent-glow)" : "none" 
            }}
          >
            {getVolumeIcon()}
          </motion.button>
      ...
      {/* PRAWY GÓRNY RÓG - Kalendarz i Ustawienia */}
      <div className="absolute top-8 right-8 flex gap-4 text-zinc-700 items-center">
        <AnimatePresence>
          {gameMode === "daily" && (
            <motion.button
        ...
        <motion.button
          ...
          className="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 hover:bg-white/5 group"
        >
          <motion.div 
          >
            <Settings size={24} />
