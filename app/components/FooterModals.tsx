...
const RulesContent = () => (
  <div className="flex flex-col gap-6 text-zinc-300 text-sm leading-relaxed">
    <p className="text-zinc-400">
      Jakitowers to polska gra muzyczna. Zgadnij piosenkę na podstawie fragmentu tekstu — im mniej prób, tym lepiej.
    </p>
    {[
    ...
    <div className="border-t border-white/5 pt-4 flex flex-col gap-1.5 text-xs text-zinc-500">
      <p>
        <span className="text-white font-bold">TRYB DZIENNY</span> — jedna nowa piosenka każdego dnia.
      </p>
      <p>
        <span className="text-white font-bold">BEZ LIMITU</span> — losowe piosenki, graj ile chcesz.
      </p>
    </div>
  </div>
);
...
const PrivacyContent = () => (
  <div className="flex flex-col gap-5 text-zinc-300 text-sm leading-relaxed">
    <p className="text-zinc-400">
      Jakitowers to niekomercyjny projekt hobbystyczny. Zależy nam na Twojej prywatności.
    </p>
    {[
      {
        title: "Dane osobowe",
        body: "Serwis nie zbiera, nie przetwarza ani nie przechowuje danych osobowych użytkowników. Nie wymagamy rejestracji, logowania ani podawania jakichkolwiek danych.",
      },
      {
        title: "Dane lokalne (localStorage)",
        body: "Wyniki gry, historia prób i preferencje (głośność, motyw kolorystyczny) zapisywane są wyłącznie lokalnie w przeglądarce — tylko na Twoim urządzeniu, nigdy na serwerze.",
      },
      {
        title: "YouTube",
        body: "Po zakończeniu rundy wyświetlamy teledysk z serwisu YouTube (Google LLC). YouTube może zbierać dane zgodnie z własną Polityką Prywatności Google, na którą nie mamy wpływu.",
      },
      {
        title: "Prawa autorskie",
        body: "Fragmenty tekstów piosenek wykorzystywane są na podstawie prawa cytatu (art. 29 ustawy o prawie autorskim i prawach pokrewnych). Celem serwisu jest rozrywka edukacyjna i popularyzacja polskiej muzyki. Użyte fragmenty mają charakter ilustracyjny i nie zastępują oryginalnych utworów. Wszelkie prawa do muzyki należą do ich twórców i wydawców.",
      },
      {
        title: "Kontakt",
        body: "W razie pytań, uwag lub zgłoszenia naruszenia praw autorskich — skontaktuj się z administratorem serwisu.",
      },
    ].map((s) => (
      <div key={s.title}>
...
const CookiesContent = () => (
  <div className="flex flex-col gap-5 text-zinc-300 text-sm leading-relaxed">
    <p className="text-zinc-400">
      Informacje o wykorzystaniu technologii przechowywania danych w serwisie Jakitowers.
    </p>
    <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4">
      <p className="text-green-400 font-bold uppercase tracking-widest text-xs mb-1">
        Brak cookies śledzących
      </p>
      <p className="text-zinc-300 text-xs">
        Jakitowers nie używa plików cookie do śledzenia, profilowania ani celów reklamowych.
      </p>
    </div>
    <div>
    ...
    <div>
      ...
      <p>
        Możesz wyczyścić wszystkie dane w ustawieniach przeglądarki w sekcji Dane witryny lub przez DevTools: F12 →
...
const ContactModal = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-7 py-5 border-b border-white/5 shrink-0 max-md:px-4 max-md:py-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] font-black tracking-[0.35em] uppercase max-md:text-[10px]" style={{ color: "var(--accent-main)" }}>
              {step === "topic" ? "NAPISZ DO NAS" : step === "sent" ? "WYSŁANO" : selectedTopic?.label.toUpperCase()}
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/10 transition-all duration-200 max-md:w-10 max-md:h-10"
          >
            <X size={15} className="max-md:w-5 max-md:h-5" />
          </button>
        </div>

        <div className="px-7 py-6 max-md:px-4 max-md:py-4">
          <AnimatePresence mode="wait">
              ...
              >
                ...
                {TOPICS.map((t) => (
                  <motion.button
                    ...
                    className="flex items-center gap-4 p-4 rounded-2xl border border-white/8 bg-white/3 hover:bg-white/6 hover:border-accent/30 transition-all duration-200 text-left group max-md:p-3 max-md:gap-3"
                  >
                    <div
                      className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-all duration-200 group-hover:scale-110 max-md:w-8 max-md:h-8"
                      style={{ background: "var(--accent-main)22" }}
                    >
                      <t.icon size={16} style={{ color: "var(--accent-main)" }} />
                    </div>
                    <div>
                      <p className="text-white font-bold text-sm max-md:text-xs">{t.label}</p>
                      <p className="text-zinc-500 text-xs mt-0.5 max-md:text-[10px]">{t.desc}</p>
                    </div>
                    <svg
                      className="ml-auto text-zinc-700 group-hover:text-zinc-400 transition-colors shrink-0"
                      width="14"
                      height="14"
                      viewBox="0 0 14 14"
                      fill="none"
            ...
            {step === "form" && (
              ...
              >
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-black tracking-[0.2em] uppercase text-zinc-500 max-md:text-[9px]">
                  ...
                  <input
                    ...
                    placeholder="Anonim"
                    maxLength={50}
                    className="bg-zinc-900/60 border border-white/10 rounded-2xl px-4 py-3 text-sm text-white placeholder:text-zinc-700 outline-none focus:border-accent/50 transition-colors max-md:px-3 max-md:py-2"
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-black tracking-[0.2em] uppercase text-zinc-500 max-md:text-[9px]">
                  ...
                  <textarea
                    ...
                    maxLength={500}
                    rows={4}
                    className="bg-zinc-900/60 border border-white/10 rounded-2xl px-4 py-3 text-sm text-white placeholder:text-zinc-700 outline-none focus:border-accent/50 transition-colors resize-none scrollbar-custom max-md:px-3 max-md:py-2"
                  />
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-3.5 rounded-2xl font-black text-sm tracking-[0.2em] uppercase transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-80 mt-1 max-md:py-3 max-md:text-xs"
            ...
            {step === "sent" && (
              ...
              >
                ...
                <button
                  onClick={onClose}
                  className="mt-2 text-[11px] font-black tracking-[0.2em] uppercase px-6 py-2.5 rounded-xl hover:opacity-80 transition-opacity max-md:text-[10px] max-md:px-4"
                  style={{ background: "var(--accent-main)", color: "#fff" }}
                >
                  ZAMKNIJ
                </button>
            ...
            {infoModal && infoContent && (
              ...
              >
                ...
                >
                  ...
                  <div className="overflow-y-auto scrollbar-custom px-7 py-6 flex-1 max-md:px-4 max-md:py-4">
                    {infoContent.body}
                  </div>
                  <div className="px-7 py-4 border-t border-white/5 shrink-0 flex items-center justify-between max-md:px-4 max-md:py-3">
                    <p className="text-[10px] text-zinc-600 tracking-wider max-md:text-[9px]">jakitowers.pl</p>
                    <button
                      onClick={() => setInfoModal(null)}
                      className="text-[11px] font-black tracking-[0.2em] uppercase px-5 py-2 rounded-xl hover:opacity-80 transition-opacity max-md:text-[10px] max-md:px-4"
                      style={{ background: "var(--accent-main)", color: "#fff" }}
                    >
                      ZAMKNIJ
                    </button>
